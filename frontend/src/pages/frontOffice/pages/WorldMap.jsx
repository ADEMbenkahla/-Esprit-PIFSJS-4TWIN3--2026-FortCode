...
export default function WorldMap() {
  ...
  const getIcon = (order) => {
    switch (order) {
      ...
      case 3:
        return <Shield className="w-6 h-6 text-amber-700" />;
      default:
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 overflow-auto">
      <div
        ...
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1734456031989-71d98f5d9807?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkYXJrJTIwZmFudGFzeSUyMGZvcnRyZXNzJTIwbWFwJTIwYWVyaWFsJTIwdmlld3xlbnwxfHx8fDE3NzA5NzkzNzd8MA&ixlib=rb-4.1.0&q=80&w=1080')`,
        }}
      >
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto py-20 flex flex-col items-center justify-center">
        ...
        {stages.map((stage, index) => {
          const isLocked = stage.participantStatus === "locked";
          const isCompleted = stage.participantStatus === "completed";
          return (
            <motion.div
              key={stage._id}
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {isLocked && stage.prerequisiteTitle && (
                      <p className="text-xs text-amber-400/90 mt-1">Requires: {stage.prerequisiteTitle}</p>
                    )}
                    {isCompleted && (
                      <div className="mt-2 inline-flex items-center gap-2 rounded-full border border-emerald-400/40 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-300">
                        Castle built • {stage.progress?.progressPercent ?? 100}% complete
                      </div>
                    )}
                  </div>
