...
def start_service(service_name, script_path, port):
    ...
    if not service_dir.exists():
        print(f"❌ Dossier {service_dir} introuvable")
        return False
    ...
    try:
        ...
        if requirements_file.exists():
            ...
            if install_result.returncode != 0:
                print(f"❌ Erreur installation dépendances {service_name}: {install_result.stderr}")
                return False
            print(f"✅ Dépendances {service_name} installées")
        ...
        if process.poll() is None:
            print(f"✅ {service_name} démarré avec succès (PID: {process.pid})")
            return True
        else:
            print(f"❌ {service_name} a échoué au démarrage")
            return False
    ...
    for service in services:
        ...
        if success:
            ...
            for process in processes:
                if process.poll() is None:
                    process.terminate()
            print(f"⚠️ Arrêt des services dû à l'erreur de {service['name']}")
            return 1
